...
df_3 = pd.read_excel(r'C:\Users\jbmk\OneDrive - Ramboll\Desktop\SoB - Doc\File Renaming PY\File_naming.xlsx', sheet_name='Drawing_num')
...
# Refferencing columns to variables (Drawing number):
df_3_Pnum = df_3['Tavlenummer']
df_3_loeg_num = df_3['Løgstrup nummer']
df_3_draw_num = df_3['Tegn. nummer']

# Defining numbering
panel_num = output[0]
for index in range(len(df_3)):      #if tiv = official drawing numbering
    if panel_num == df_3_Pnum[index]:
        tiv = df_3_draw_num[index]
        break
# for index in range(len(df_3)):      #if tiv = loegstrup numbering
#     if panel_num == df_3_Pnum[index]:
#         tiv = df_3_loeg_num[index]
#         break
...
def drawing_namer(fname):
    for index in range(len(df)):
            ...
            #03-833xx filer:
                ...
                numbers = re.findall(r"\d{4}\.|\d{2}\.|\d{2}\.", fname)
                dnum = ''.join(numbers)
...
def file_namer(fname):
    for index in range(len(df_2)):
            ...
            #03-833xx filer:
                ...
                numbers = re.findall(r"\d{4}\.|\d{2}\.|\d{2}\_", fname)
                dnum = ''.join(numbers)
